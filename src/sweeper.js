...
BlockIoSweep.DEFAULT_BLOCKCHAIN_PROVIDER_API_URL =
  constants.BLOCKCHAIN_PROVIDER_URL_DEFAULT
...
function getCoinValue(floatAsString) {
  ...
  const r = parseInt(
    '' + s[0] + s[1] + constants.COIN.substr(1, 8 - s[1].length)
  )
...
async function createBalanceMap(
  ...
  for (let i = 0; i <= n; i++) {
    ...
    if (
      network !== constants.NETWORKS.DOGE &&
      network !== constants.NETWORKS.DOGETEST
    ) {
